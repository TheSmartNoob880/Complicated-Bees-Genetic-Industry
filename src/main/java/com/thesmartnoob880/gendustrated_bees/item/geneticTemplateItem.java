       ...
       boolean heldShift = Screen.hasShiftDown();
           ComplicatedBees.GENE_REGISTRY.get().getEntries().forEach((entry)->{
               ...
               if (pStack.getTag().contains(geneKey)){
                   ...
                   if (heldShift)
                       value = Component.translatable("item.gendustrated_bees.genetic_template.dominance."+deseralizedGene.isDominant()).withStyle(Style.EMPTY.withColor(deseralizedGene.isDominant() ? 15086117 : 2457574));
                   ...
                   ptooltipComponents.add(Component.translatable(transKey+"_label").append(": ")
                           ...
                           .append(Component.translatable("item.gendustrated_bees.genetic_template.empty").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY)));
               }
           });
           if (!heldShift&&containsOneOrMoreGenes(pStack)) {
               ptooltipComponents.add(Component.translatable("item.gendustrated_bees.genetic_template.more_info").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY));
           }
    }
    ...
    public Component getName(ItemStack stack) {
        if (stack.hasTag()){
            if (isCompleteTemplate(stack)){
                return Component.translatable("item.gendustrated_bees.genetic_template.complete");
            } else if (containsOneOrMoreGenes(stack)){
                return Component.translatable("item.gendustrated_bees.genetic_template.incomplete");
            } else{
                return Component.translatable("item.gendustrated_bees.genetic_template.blank");
            }
        } else{
            return Component.translatable("item.gendustrated_bees.genetic_template.blank");
        }
