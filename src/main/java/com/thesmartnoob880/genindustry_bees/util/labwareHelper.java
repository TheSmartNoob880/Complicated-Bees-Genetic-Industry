...
public class labwareHelper {
    ...
    public static void register(Item item, Double pdouble){
        if (labwareExtractionRates.containsKey(item))
            GenIndustryBees.LOGGER.warn("item{} is already registered as labware. Previous Rate:{}, New Rate:{}", item.getDescriptionId(), labwareExtractionRates.get(item), pdouble);
